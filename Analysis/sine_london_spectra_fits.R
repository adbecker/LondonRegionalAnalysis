...
nms <- c('all','north','south','east','west','central')
...
q.all <- q.all + ylab('normalized power')
q.central <- q.central + ylab('normalized power')
q.west <- q.west +theme(legend.title = element_text(NULL),legend.position = 'top')+
  theme(
...
p <- plot_grid(q.all, q.south, q.west, q.central ,q.east ,q.north, align = 'hv',labels = 'AUTO')

#jpeg(file='London_sine_fitted_spectra.jpg',height=6,width=8,units="in",res=300)
print(p)
#dev.off()
